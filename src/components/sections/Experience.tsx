...
export default function Experience() {
  ...
  return (
    <section id="experience" className="bg-black py-24 px-4">
      <div className="max-w-5xl mx-auto" ref={ref}>
        ...
        >
          <span className="text-sm font-semibold tracking-widest text-purple-400 uppercase">Experience</span>
          <h2 className="mt-2 text-4xl font-bold text-white">Where I've worked</h2>
        </motion.div>
        ...
        <div className="relative">
          ...
          <div className="space-y-10">
            {jobs.map((job, i) => (
              ...
              >
                {/* Timeline dot */}
                <div className={`absolute left-2.5 top-5 h-3 w-3 rounded-full border-2 ${job.border} bg-black hidden md:block`} />

                <div className={`rounded-xl border ${job.border} bg-zinc-900 p-6 hover:bg-zinc-800/50 transition-colors duration-300`}>
                  <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
                    ...
                    <div className="text-right">
                      ...
                      <br />
                      <span className="text-xs rounded-full border border-zinc-700 px-2 py-0.5 text-zinc-500">
                        {job.type}
                      </span>
                    </div>
                  </div>
        ...
        <motion.div
          ...
          className="mt-16"
        >
          <h3 className="text-lg font-semibold text-white mb-6">Education & Certifications</h3>
          <div className="grid sm:grid-cols-2 gap-4">
            <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-5">
              <p className="text-xs text-purple-400 uppercase tracking-widest mb-1">University</p>
              <p className="font-semibold text-white">Bachelor of Computer Science</p>
              <p className="text-sm text-zinc-400">University of Salzburg · 2019–2023</p>
            </div>
            {certs.map((c) => (
              <div key={c.name} className="rounded-xl border border-zinc-800 bg-zinc-900 p-5">
                <p className="text-xs text-purple-400 uppercase tracking-widest mb-1">{c.issuer} · {c.year}</p>
